...
describe("GET /logs", () => {
  it("should return 200 OK with an array of logs", async () => {
    const mockLogs = [
      { id: 1, json: { message: "test" }, inserted_at: new Date() },
    ];
    ...
    expect(res.body).toEqual(
      mockLogs.map((log) => ({
        ...log,
        inserted_at: log.inserted_at.toISOString(),
      }))
    );
  });
...
describe("POST /logs", () => {
  it("should return 201 Created with the new log", async () => {
    ...
    expect(res.body).toEqual({
      ...returnedLog,
      inserted_at: returnedLog.inserted_at.toISOString(),
    });
  });
